...
- **Species detection**: Automatically identifies organism (human, mouse, zebrafish, drosophila) from gene naming patterns
- **Experimental context reasoning**: Understands biological meaning of factors (treatments, timepoints, genotypes) and infers research questions
- **Composite factor parsing**: Automatically detects and parses factors encoding multiple dimensions (e.g., "DietTimepoint" → Diet × Timepoint)
...
- **Automatic visualization**: Generates appropriate diagrams (trees for nested, grids for factorial crossed designs)
- **Experiment card generation**: Produces markdown documentation with design structure, biological context, analysis recommendations, and metadata
...
- **GRAMMAR.md** - Complete edviz grammar specification (referenced by skill during design analysis)
...
- **Python Scripts:**
  ...
  - `scripts/design_to_grammar.py` - Convert design structure to edviz grammar notation (handles crossed factors, composite parsing)
  - `scripts/generate_experiment_card.py` - Generate markdown experiment card with biological context and analysis recommendations
...
5. **Detects species** from gene naming patterns in .var slot
6. **Reasons about experimental context** - understands biological meaning of factors, identifies treatments/controls, infers research questions
7. **Parses composite factors** - detects factors encoding multiple dimensions (e.g., "Hepatocyte_15weeks" → CellFraction × DietTimepoint)
8. Detects design structure (nested vs crossed vs factorial)
9. Generates edviz grammar string representing the complete design
10. Renders visualizations using edviz (trees, grids, or mixed layouts)
11. Creates an experiment card (markdown documentation with biological context and statistical guidance)
12. Shows distribution summaries with appropriate labels (samples per treatment, cells per sample, etc.)
...
This appears to be a tumor study comparing genetic perturbations.
- genotype: Two conditions (KeapKO vs KeapWT) - KEAP1 knockout vs wildtype
- sample: Biological replicates nested within genotypes
- cell_type: Tumor microenvironment profiling (6 cell populations)

Research Question: How does KEAP1 loss affect tumor microenvironment composition?
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Samples per Genotype: 2 samples each
Cells per Sample: 5,103 - 7,904 (mean: 5,356)
Cells per CellType: 740 - 9,346 (varies by population)
```

## Design Types Supported

- **Nested/Hierarchical**: Samples nested within conditions (e.g., `Genotype > Sample`)
- **Crossed/Factorial**: All combinations of factors exist (e.g., `Genotype × Treatment`)
- **Mixed**: Combination of nested and crossed factors (e.g., `(CellFraction × DietTimepoint) > Sample`)
- **Composite factors**: Automatically parses factors encoding multiple dimensions (e.g., "Hepatocyte_15weeks" → CellFraction × DietTimepoint)
...
This skill includes a copy of edviz's GRAMMAR.md for offline reference during design analysis. The skill agent consults this file when:
- Validating complex grammar constructs
- Understanding operator precedence rules
- Handling edge cases in unbalanced designs
- Resolving ambiguities in nested vs crossed relationships

This ensures accurate grammar generation even for complex multi-factor designs.
